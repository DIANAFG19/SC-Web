function initMap() {    	 
 	  //Mapa creado para el registro de incidentes
     ...
     //AGREGAR UN MARCADOR QUE PODRÁ SER ARRASTRADO POR EL USUARIO               
        //var mkReg = 'http://i793.photobucket.com/albums/yy211/Roi_7/mkRegistrar.png'; //icono
        var mkReg = document.getElementById("imgReg").src;
                       
       var markerRegistrar = new google.maps.Marker({
			map: map,
			position: objLimites.getCenter(),
			icon: mkReg,
			draggable: true
		});			
		var markerPosition;
		google.maps.event.addListener(markerRegistrar, 'dragstart', function() {
			markerPosition = this.getPosition();
		});
     
	 //Evento que escuchará cada instante en el que se ARRASTRE el marcador
     google.maps.event.addListener(markerRegistrar, 'dragend', function() {
    	//Se obtienen las coordenadas y se guardan en el id de lat y lng del formulario
 	    document.getElementById("lat").value = markerRegistrar.getPosition().lat();
       	document.getElementById("lng").value = markerRegistrar.getPosition().lng();
    	
    	//Si el polígono de la UAM Azc NO CONTIENE la posición del marcador, mostrará un mensaje de alerta
    	if (google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoUam) == false) {
        	alert("Has arrastrado el marcador fuera de los l\u00EDmites permitidos, debe ser dentro de la regi\u00F3n de la UAM Azcapotzalco.");
        	//Se asigna la última posición antes de salir de la región de la UAM
        	this.setPosition(markerPosition);        
    	}
    	//Si el polígono B CONTIENE la posición del marcador
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoB) == true){
        	//Se abrirá una ventana de información indicando las coordenadas y espacio donde está el marcador  
    	    var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio B</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);
    		//Y a su vez se cerrará la ventana anterior cuando se arrastre el marcador a una nueva posición
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		//Se asigna el id del espacio físico en el formulario
    		document.getElementById("idEspacioFK").value = 1;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoC) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio C</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 2;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoD) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio D</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 3;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoE) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio E</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 4;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoF) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio F</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 5;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoG) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio G</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 6;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoGbis) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio G-Bis</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 7;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoH) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio H</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 8;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoHO) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio HO</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 9;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoHP) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio HP</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 10;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoI) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio I</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 11;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoJ) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio J</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 12;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoK) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio K</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 13;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoL) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio L</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 14;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoM) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio M</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 15;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoO) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio O</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 16;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoP) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio P</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 17;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoP1) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio P1</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 18;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoP2) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio P2</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 19;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoP3) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio P3</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 20;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoP4) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio P4</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 21;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoQ) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio Q</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 22;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoR) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio R</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 23;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoS) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio S</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 24;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoT) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio T</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 25;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoW) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Edificio W</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 26;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoPEgresados) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Plaza de Egresados</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 27;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoPRoja) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Plaza Roja</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 28;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoPBiblioteca) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Plaza de Biblioteca</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 29;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoADeportiva) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacuterea Deportiva</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 30;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoCBasquet) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Canchas de Basquetball</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 31;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoCFutbol) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Cancha de F&uacutetbol R&aacutepido</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 32;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEProfsNE) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Estacionamiento de Profesores NorEste</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 33;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEProfsSur) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Estacionamiento de Profesores Sur</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 34;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEAlumsSO) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Estacionamiento de Alumnos SurOeste</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 35;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEAlumsSE) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Estacionamiento de Alumnos SurEste</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 36;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEntrada2) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Entrada 2</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 37;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEntrada4) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Entrada 4</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 38;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEntrada5) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Entrada 5</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 39;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEntrada6) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Entrada 6</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 40;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoEntrada7) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Entrada 7</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 41;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoKioskoB) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Kiosko B</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 42;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoKioskoCanchas) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>Kiosko Canchas</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 43;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesB) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes B</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 44;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesCD) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes C/D</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 45;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesD) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes D</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 46;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesE) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes E</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 47;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesGI) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes G/I</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 48;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesHHP) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes H/HP</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 49;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesIT) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes I/T</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 50;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesJL) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes J/L</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 51;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesK) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes K</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 52;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesPW) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes P/W</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 53;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesPR) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes P/R</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 54;
        }
        if(google.maps.geometry.poly.containsLocation(this.getPosition(), poligonoAVerdesRCB) == true){
        	var infowindow = new google.maps.InfoWindow({
    	    	content: '<p>Coordenadas: ' + markerRegistrar.getPosition() + '</p>' + '<p><center><strong>&Aacutereas Verdes R/Canchas Basquet</strong></center></p>'});
    		infowindow.open(map, markerRegistrar);    		
    		google.maps.event.addListener(markerRegistrar,'drag',function(){ 
    			infowindow.close(map, markerRegistrar);});
    		document.getElementById("idEspacioFK").value = 55;
        }  	
     });     
}
